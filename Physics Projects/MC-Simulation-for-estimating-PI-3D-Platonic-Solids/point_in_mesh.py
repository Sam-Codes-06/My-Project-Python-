...
def check_point_intersects_face(p, plane, limit):
    ...
    if len(plane) > 3:
        t += np.arccos(round(angle[3][0] / (angle[3][1] * angle[3][2]), 13)) + np.arccos(round(angle[4][0] / (angle[4][1] * angle[4][2]), 13))
...
def check_point_inside_solid(face_indices, points, point):
